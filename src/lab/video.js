...
class FF {
  ...
  createOutputPath (inputPath, type = 'resize', hardExtname = '.mp4') {
    if (inputPath) {
      ...
      return {
        output: path.join(
          dirname,
          basename.replace(extname, '') + `_${type}` + (hardExtname || extname)
        ),
        dirname,
        basename,
        extname
  ...
  getFileType (formatName = null) {
    ...
    if (
      Array.from(['mov', 'm4v', 'avi', 'mkv', 'mp4', 'webm'], t =>
        formatName.match(t) ? 1 : null
      ).filter(f => f).length > 0
    )
      return 'video'
    if (
      Array.from(['mp3'], t => (formatName.match(t) ? 1 : null)).filter(f => f)
        .length > 0
    )
      return 'audio'
    if (
      Array.from(['gif'], t => (formatName.match(t) ? 1 : null)).filter(f => f)
        .length > 0
    )
      return 'gif'
  }
  ...
  videoExtract (input, size, progressFn, fadeIn = 10, fadeOut = 10, fps = 24) {
    return new Promise((resolve, reject) => {
      let { dirname, basename, extname } = this.createOutputPath(
        input,
        'extract'
      )
  ...
  audioCut (audio, duration = null, startTime = 0, loop = 5) {
    ...
    let format = 'mp3'
    let { output } = this.createOutputPath(
      audio,
      `cut_${loop.toString().replace('.', '_')}`,
      '.' + format
    )
    return new Promise((resolve, reject) => {
      ffmpeg()
  ...
  audioCutAuto (audio, loop = 5) {
    return new Promise((resolve, reject) => {
      this.getMediaDurationAndType(audio).then(({ duration }) => {
        this.audioCut(audio, duration, 0, loop)
          .then(output => {
            resolve(output)
          })
          .catch(error => {
  ...
  video2mp4 (filePath, startTime = 0, loop = 5) {
    let { output } = this.createOutputPath(filePath, 'output', '.mp4')
    return new Promise((resolve, reject) => {
      this.getMediaDurationAndType(filePath).then(info => {
    ...
    return new Promise((resolve, reject) => {
      if (content.type == 'img') {
        ...
        this.singleImg2video(url, width + 'x' + height, loop).then(imvot => {
          // imvot 图片转视频
          this.paddingVideo(imvot, parseInt(width), parseInt(height), [
            tw,
            th,
            parseInt(content.layout.left),
            parseInt(content.layout.top)
          ]).then(pot => {
            // 删除
            ...
            if (textImage && textImage.base64) {
              ...
              this.drawText(
                pot,
                textImage.base64,
                textImage.layout.left,
                textImage.layout.top
              ).then(textVideo => {
                // 删除
                this.deleteFile(pot)
                resolve(textVideo)
        ...
        this.paddingVideo(url, parseInt(width), parseInt(height), [
          tw,
          th,
          0,
          parseInt(content.layout.top)
  ...
  mergeAll (data, progressFn) {
    ...
    return new Promise((resolve, reject) => {
      ...
      let { output } = this.createOutputPath(
        path.join(__dirname, '../test/video'),
        'v'
      )
      this.files2video(
        files,
        ...
        if (backgroudAudio && backgroudAudio.url) {
          ...
          this.getMediaDurationAndType(video).then(info => {
            // 裁切音乐
            this.audioCutAuto(backgroudAudio.url, ~~info.duration || 1).then(
              audio => {
                // 删除audio
                // console.log(audio)
                // 添加音乐
  ...
  screenshots (filePath, count = 4) {
    let { output } = this.createOutputPath(filePath, '', 'output')
    return new Promise((resolve, reject) => {
      ffmpeg(filePath)
