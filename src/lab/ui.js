...
class UI {
  ...
  createFeed (items = []) {
    ...
    div.innerHTML = Array.from(
      items,
      item => `<div class="event" data-item='${JSON.stringify(item)}'>
            <div class="label">
            ...
            ${
              item.like
                ? `<a class="like"><i class="like icon"></i>${item.like}</a>`
                : ''
            }
            </div>
            </div>
  ...
  createModel (header = '', content = '') {
    ...
    div.add = function (child, type = 'content') {
      if (div.querySelector(`.${type}`))
        div.querySelector(`.${type}`).appendChild(child)
    }
    div.clear = function (type = 'content') {
      if (div.querySelector(`.${type}`))
        div.querySelector(`.${type}`).innerHTML = ''
    }
    div.show = function () {
      $(div).modal('show')
    }
  ...
  createTags (tags = []) {
    ...
    div.innerHTML = Array.from(
      tags,
      t => `<a class="item" data-id='${String(t.id) || ''}'>
            <div class="ui ${
              t.color ? t.color : ''
            } horizontal label" data-id='${String(t.id) || ''}'>${
        t.label ? t.label : t
      }</div>
        </a>`
    ).join('')
  ...
  createAccordion (items = []) {
    ...
    div.innerHTML = Array.from(items, i => {
      return `<div class="item">
                        <a class="${
                          i.active ? 'active' : ''
                        } title"><i class="dropdown icon"></i>${i.title}</a>
                        <div class="${i.active ? 'active' : ''} content">
                            <div class="ui form">
                                <div class="grouped fields">
                                ${Array.from(
                                  i.data,
                                  t => `<div class="field">${t}</div>`
                                ).join('')}
                                </div>
                            </div>
                        </div>
  ...
  loading (n = 0) {
    if (!this.loadingElement) {
      ...
      this.loadingElement.className =
        'ui active slow black double inverted loader'
    ...
    let btn = this.createIcon(
      ...
      e => {
        ...
        if (!btn.classList.contains('fan')) {
          ...
          ;(btn.querySelector('svg') || btn.querySelector('i')).classList.add(
            'fa-spin'
          )
        } else {
  ...
  createTextInput (text, key, eventListener = null) {
    ...
    div.querySelector('p').outerHTML = `<label>${
      ...
      async () => {
        if (video.getAttribute('data-status') === 'play') {
          ...
          if (
            ...
            video.querySelector('video').srcObject.getTracks()
          ) {
            video
              .querySelector('video')
          ...
          let v = video.querySelector('video')
            ? video.querySelector('video')
            : await this.createVideo(null, false)
  ...
  createTextCanvas (txt, style, isAdd = false) {
    ...
    canvas.update = function (textNew) {
      ...
      canvas.height =
        font.fontBoundingBoxAscent + font.fontBoundingBoxDescent + 12
  ...
  createShortVideoInput () {
    ...
    if (filePaths && filePaths.length > 0) {
      for (const url of filePaths) {
        ...
        var count = Array.from(['mov', 'm4v', 'avi', 'mkv', 'mp4', 'webm'], t =>
          urlNew.match(t) ? 1 : null
        ).filter(f => f)
        if (count.length > 0) type = 'video'
        count = Array.from(['mp3'], t => (urlNew.match(t) ? 1 : null)).filter(
          f => f
        )
        if (count.length > 0) type = 'audio'
        count = Array.from(['jpeg', 'jpg', 'png', 'webp'], t =>
          urlNew.match(t) ? 1 : null
        ).filter(f => f)
        if (count.length > 0) type = 'img'
        count = Array.from(['gif'], t => (urlNew.match(t) ? 1 : null)).filter(
          f => f
        )
